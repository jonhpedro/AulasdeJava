...
public class ServletLogin extends HttpServlet {
	...
	private void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (new ClienteService().efetuarLogin(usuario) != null) {
			session.setAttribute("cliente", new ClienteService().efetuarLogin(usuario));
		...
		if(new FuncionarioService().efetuaLogin(usuario) != null) {
			session.setAttribute("cliente", new FuncionarioService().efetuaLogin(usuario));
			response.sendRedirect("cliente/home.jsp");
	}else {
		request.setAttribute("msg", "Falha ao efetuar o login");
		request.getRequestDispatcher("index.jsp").forward(request, response);
